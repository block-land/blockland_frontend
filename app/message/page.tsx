...
function MessagePageInner() {
  ...
  // Global search results (new contacts from the backend)
  const [globalResults, setGlobalResults] = useState<SearchedUser[]>([]);
  const [searchingGlobal, setSearchingGlobal] = useState(false);

  // Debounced search for new users/contacts from backend
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const res = await searchUsers(query, currentWallet);
      if (res.ok) {
        // Exclude users we already have conversations with
        const filtered = res.users.filter(
          (u) => !conversations.some((c) => c.other.walletAddress === u.walletAddress)
        );
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setPendingRecipient({
        ...
        username: shortWallet(sellerWallet),
        photoUrl: null,
      });
    }
        ...
        <div className="w-full md:w-[350px] shrink-0 bg-zinc-950 border border-zinc-900 rounded-3xl flex flex-col overflow-hidden">
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${
                    connected ? "bg-emerald-550" : "bg-zinc-600"
                  }`}
                />
          ...
          <ScrollArea className="flex-1">
            <div className="p-3 space-y-1">
              ...
              ) : filteredThreads.length === 0 && globalResults.length === 0 ? (
                ...
                <>
                  ...
                  {filteredThreads.length > 0 && (
                    <div className="space-y-1">
                      {searchQuery.trim().length >= 2 && (
                        <div className="text-[10px] uppercase font-semibold text-zinc-500 px-3 py-1">
                          Existing Chats
                        </div>
                      )}
                      {filteredThreads.map((thread) => (
                        <ConversationListItem
                          key={thread.id}
                          thread={thread}
                          active={thread.id === activeId && !pendingRecipient}
                          onSelect={() => {
                  ...
                  {/* Global search results (new contacts) */}
                  {globalResults.length > 0 && (
                    <div className="space-y-1 mt-4">
                      <div className="text-[10px] uppercase font-semibold text-zinc-500 px-3 py-1 flex items-center justify-between">
                        <span>Other Contacts</span>
                        {searchingGlobal && <Loader2 className="h-3 w-3 animate-spin" />}
                      </div>
                      {globalResults.map((user) => (
                        <ContactListItem
                          key={user.walletAddress}
                          user={user}
                          active={pendingRecipient?.walletAddress === user.walletAddress}
                          onSelect={() => {
                            setPendingRecipient({
...
/** Contact list item for searched new users. */
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        <ParticipantAvatar photoUrl={user.photoUrl} name={user.username} size={40} />
